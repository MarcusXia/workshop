...
function insertMailingListEntry() {
	...
	try {
		...
		// Insert new record
		query = "INSERT INTO \"WORKSHOP\".\"workshop.admin.data::mailingList\"  values(\"WORKSHOP\".\"workshop.admin.data::recordId\".NEXTVAL, ?, ?, now(), ?, ?, ?)";
...
function deleteAllMailingListEntries() {
	...
	try {
		...
		// Insert new record
		query = "TRUNCATE TABLE \"WORKSHOP\".\"workshop.admin.data::mailingList\"";
...
function downloadExcel() {
	...
	try {
		var query = 'SELECT \"RecordId\", \"SessionId\", \"UserId\", \"DateCreated\", \"FirstName\", \"LastName\" , \"EmailAddress\" '
				+ 'FROM \"WORKSHOP\".\"workshop.admin.data::mailingList\"  order by \"RecordId\"';
		$.trace.debug(query);
		var conn = $.hdb.getConnection();
		...
		for(var i=0; i<rs.length;i++){
			body += rs[i].RecordId + "\t" + rs[i].SessionId + "\t"
					...
					+ rs[i].FristName + "\t" + rs[i].LastName + "\t" + rs[i].EmailAddress + "\n";
		}
	} catch (e) {
		$.response.status = $.net.http.INTERNAL_SERVER_ERROR;
