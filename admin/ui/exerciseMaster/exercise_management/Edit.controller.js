...
//   }
	 ...
	 loadPackageFilter: function(oEvent,oController){
		   gSearchParam = oEvent.getParameter("suggestValue");
		    var aUrl = '/workshop/admin/services/exercisesMaster.xsjs?cmd=getPackages&package='+escape(gSearchParam);
		    jQuery.ajax({
		       url: aUrl,
		       method: 'GET',
		       dataType: 'json',
	 ...
	 onLoadFilter: function(myJSON){
		  ...
		  for( var i = 0; i<myJSON.Packages.length; i++)
		     {
			  oSearchControl.addItem(new sap.ui.core.ListItem({text: myJSON.Packages[i].PACKAGE_ID}));
		 ...
		 loadObjectFilter: function(oEvent,oController){
			   ...
			   packageName = sap.ui.getCore().byId("nodePackage").getValue();
			    var aUrl = '/workshop/admin/services/exercisesMaster.xsjs?cmd=getObjList&package='+escape(packageName)+'&obj='+gSearchParam;
			    jQuery.ajax({
			       url: aUrl,
			       method: 'GET',
			       dataType: 'json',
		 ...
		 onLoadObjFilter: function(myJSON){
			  ...
			  oSearchControl.destroyItems();
			  for( var i = 0; i<myJSON.Objects.length; i++)
			     {
				  oSearchControl.addItem(new sap.ui.core.ListItem({text: myJSON.Objects[i].OBJECT_NAME}))
