//To use a javascript controller its name must end with .controller.js
sap.ui.controller("odataView.App", {
	 onInit : function(){
         this.getView().addStyleClass("sapUiSizeCompact"); // make everything inside this View appear in Compact mode

         var userName = sap.ui.getCore().getModel("config").getProperty("/UserName");
         var urlBP = "/workshop/exercises/g<group>/services/businessPartners.xsodata";
         var urlMulti = "/workshop/exercises/g<group>/services/purchaseOrders.xsodata";
         var urlCreate = "/workshop/exercises/g<group>/services/user.xsodata";
		 var parts = userName.split("_");
	     var grpNumber = parts[1];
	     if (typeof(grpNumber) != 'undefined' && grpNumber != null){
	    	urlBP = urlBP.replace(/<group>/g,grpNumber);
	    	urlMulti = urlMulti.replace(/<group>/g,grpNumber);	
	    	urlCreate = urlCreate.replace(/<group>/g,grpNumber);	    	
	   	 }
	     
	     sap.ui.getCore().getModel().setProperty("/bpPath",urlBP);
	     sap.ui.getCore().getModel().setProperty("/bpEntity","/BusinessPartners");
	     sap.ui.getCore().getModel().setProperty("/mPath",urlMulti);
	     sap.ui.getCore().getModel().setProperty("/mEntity1","/POHeader");	
	     sap.ui.getCore().getModel().setProperty("/mEntity2","/POItem");	     
	     sap.ui.getCore().getModel().setProperty("/cPath",urlCreate);
	     sap.ui.getCore().getModel().setProperty("/cEntity","/Users");
	     
	     
     },
     callSimpleService: function(){
    	  var oTable = sap.ui.getCore().byId("tblHeader");
		  oTable.removeAllColumns();
		  oTable.removeAllItems();
		  var bpPath = sap.ui.getCore().getModel().getProperty("/bpPath");
		  var bpEntity = sap.ui.getCore().getModel().getProperty("/bpEntity");
	   	  var oModel = new sap.ui.model.odata.ODataModel(bpPath, true);
	   	  oModel.attachEvent("requestFailed",oDataFailed)
	   	  
	   	  var oMeta = oModel.getServiceMetadata();
	   	  var oControl;
          var columnList = new sap.m.ColumnListItem();
	   	  if(!oMeta){
	   	  		 sap.m.MessageBox.show("Bad Service Definition", 
		   	   	 			{ icon: sap.m.MessageBox.Icon.ERROR, 
	   	 			          title: "Service Call Error",
	   	 			          actions: [sap.m.MessageBox.Action.OK],
	   	 			          styleClass: "sapUiSizeCompact" } ); }
	   	  else{
	   		//Table Column Definitions 
            for ( var i = 0; i < oMeta.dataServices.schema[0].entityType[0].property.length; i++) {
	                var property = oMeta.dataServices.schema[0].entityType[0].property[i];
	                oTable.addColumn(new sap.m.Column({
	                    header: new sap.m.Label({
	                        text: property.name
	                    }),
	                    width: "125px"
	                }));
	                columnList.addCell(new sap.m.Text({
	                    text: {
	                        path: property.name
	                    },
	                    name: property.name
	                }));
	            }
	   	  	oTable.setModel(oModel);
	   	  	oTable.bindItems({
              path: bpEntity,
              template: columnList
	   	  	}); 
	   	 }
     },
     
     callMultiService: function(){
      	  var oTable = sap.ui.getCore().byId("tblPOHeader");
    	  var oTableItem = sap.ui.getCore().byId("tblPOItem");		    	  
		  oTable.removeAllColumns();
		  oTable.removeAllItems();
		  oTableItem.removeAllColumns();
		  oTableItem.removeAllItems();
		  
		  var mPath = sap.ui.getCore().getModel().getProperty("/mPath");
		  var mEntity1 = sap.ui.getCore().getModel().getProperty("/mEntity1"); 
		  var mEntity2 = sap.ui.getCore().getModel().getProperty("/mEntity2");		  
		  
	   	  var oModel = new sap.ui.model.odata.ODataModel(mPath, true);
	   	  oModel.attachEvent("requestFailed",oDataFailed)
	   	  
	   	  var oMeta = oModel.getServiceMetadata();
          var oControl;
          var columnList = new sap.m.ColumnListItem();
          var columnListItem = new sap.m.ColumnListItem();          
          if(!oMeta){
	   	  		 sap.m.MessageBox.show("Bad Service Definition", 
		   	   	 			{ icon: sap.m.MessageBox.Icon.ERROR, 
	   	 			          title: "Service Call Error",
	   	 			          actions: [sap.m.MessageBox.Action.OK],
	   	 			          styleClass: "sapUiSizeCompact" } ); }
          else{
        	//Table Column Definitions 
              for ( var i = 0; i < oMeta.dataServices.schema[0].entityType[0].property.length; i++) {
  	                var property = oMeta.dataServices.schema[0].entityType[0].property[i];
  	                oTable.addColumn(new sap.m.Column({
  	                    header: new sap.m.Label({
  	                        text: property.name
  	                    }),
  	                    width: "125px"
  	                }));
  	                columnList.addCell(new sap.m.Text({
  	                    text: {
  	                        path: property.name
  	                    },
  	                    name: property.name
  	                }));
  	            }	   	  
	   	  oTable.setModel(oModel);
	   	  for ( var i = 0; i < oMeta.dataServices.schema[0].entityType[1].property.length; i++) {
	   		  var property = oMeta.dataServices.schema[0].entityType[1].property[i];
	   		  
	   		oTableItem.addColumn(new sap.m.Column({
                  header: new sap.m.Label({
                      text: property.name
                  }),
                  width: "125px"
              }));
              columnListItem.addCell(new sap.m.Text({
                  text: {
                      path: property.name
                  },
                  name: property.name
              }));	   		  
	   		  
	        	     
	   	  }		   	  
	
	   	  oTableItem.setModel(oModel);
          }		   	  
	   	  oTable.bindItems({
            path: mEntity1,
            template: columnList
	   	  	});    
	   	  oTableItem.bindItems({
	            path: mEntity2,
	            template: columnListItem
		   	  	});  	   	  
     },
     
     onRowSelect: function(oEvent){
			var data = oEvent.getSource().getModel();
			var oTable = oEvent.getSource();
			var poId = data.getProperty("PURCHASEORDERID",oTable.getContextByIndex(oTable.getSelectedIndex()));
			
			var oTableItems = sap.ui.getCore().byId("tblPOItem");
			var ContextItem = sap.ui.getCore().byId("val2_2").getValue()+"(PURCHASEORDERID='"+poId+"')"+sap.ui.getCore().byId("val2_3").getValue();
			oTableItems.bindItems(ContextItem);    	 
     },
     
     callUserService: function(){
		  var oController = sap.ui.getCore().byId("App").getController();
		  var oTable = sap.ui.getCore().byId("tblUsers");
		  oTable.removeAllColumns();
		  oTable.removeAllItems();
		  
		  var cPath = sap.ui.getCore().getModel().getProperty("/cPath");
		  var cEntity = sap.ui.getCore().getModel().getProperty("/cEntity"); 
		  
	   	  var oModel = new sap.ui.model.odata.ODataModel(cPath, true);
	   	  oModel.attachEvent("requestFailed",oDataFailed)
	   	  
	   	  var oMeta = oModel.getServiceMetadata();
          var oControl;
          var columnList = new sap.m.ColumnListItem();
          if(!oMeta){
	   	  		 sap.m.MessageBox.show("Bad Service Definition", 
		   	   	 			{ icon: sap.m.MessageBox.Icon.ERROR, 
	   	 			          title: "Service Call Error",
	   	 			          actions: [sap.m.MessageBox.Action.OK],
	   	 			          styleClass: "sapUiSizeCompact" } ); }	
         else{
        	 for ( var i = 0; i < oMeta.dataServices.schema[0].entityType[0].property.length; i++) {
	                var property = oMeta.dataServices.schema[0].entityType[0].property[i];
	                oTable.addColumn(new sap.m.Column({
	                    header: new sap.m.Label({
	                        text: property.name
	                    }),
	                    width: "125px"
	                }));
	                columnList.addCell(new sap.m.Text({
	                    text: {
	                        path: property.name
	                    },
	                    name: property.name
	                }));
	            }	   	  	   	  
	   	  oTable.setModel(oModel);
	   	  var sort1 = new sap.ui.model.Sorter("PERS_NO",true);
	   	  oTable.bindItems({
	            path: cEntity,
	            template: columnList,
	            sorter: sort1
		   	  	});   	   	  
         }	
         
         //Call Update Service
           var aUrl = cPath+cEntity;
           var oEntry = {};
           oEntry.PERS_NO = "0000000000";
           oEntry.FIRSTNAME = sap.ui.getCore().getModel().getProperty("/fName"); 
           oEntry.LASTNAME = sap.ui.getCore().getModel().getProperty("/lName"); 
           oEntry.E_MAIL = sap.ui.getCore().getModel().getProperty("/email"); 

			oModel.setHeaders({"content-type" : "application/json;charset=utf-8"});
			oModel.create(cEntity, oEntry, null, oController.onSuccessCall, onErrorCall);
 
     },
     
     onSuccessCall: function(){
		  var oTable = sap.ui.getCore().byId("tblUsers");
		  var oModel = oTable.getModel();
		  var cEntity = sap.ui.getCore().getModel().getProperty("/cEntity"); 		  
		  var oMeta = oModel.getServiceMetadata();
          var oControl;
          var columnList = new sap.m.ColumnListItem();
          if(!oMeta){
	   	  		 sap.m.MessageBox.show("Bad Service Definition", 
		   	   	 			{ icon: sap.m.MessageBox.Icon.ERROR, 
	   	 			          title: "Service Call Error",
	   	 			          actions: [sap.m.MessageBox.Action.OK],
	   	 			          styleClass: "sapUiSizeCompact" } ); }	
         else{
        	 for ( var i = 0; i < oMeta.dataServices.schema[0].entityType[0].property.length; i++) {
	                var property = oMeta.dataServices.schema[0].entityType[0].property[i];
	               columnList.addCell(new sap.m.Text({
	                    text: {
	                        path: property.name
	                    },
	                    name: property.name
	                }));
	            }	   	  
   	   	  var sort1 = new sap.ui.model.Sorter("PERS_NO",true);        	 
	   	  oTable.bindItems({
	            path: cEntity,
	            template: columnList,
	            sorter: sort1
		   	  	});   	   	  
         }		
	}     
});