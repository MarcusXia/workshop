$.import("workshop.admin.services", "session");
var SESSIONINFO = $.workshop.admin.services.session;
function checkSchema(){
	...
	rs = conn.executeQuery(query,schema,'%PurchaseOrder.Header' );
	for(var i = 0; i < rs.length; i++){
		   count = rs[i].ROWS;}
	if(count<1){
	...
	rs = conn.executeQuery(query,schema,'%PurchaseOrder.Item');
	for(var i = 0; i < rs.length; i++){
		   count = rs[i].ROWS;}
	if(count<1){
	...
	for(var i = 0; i < rs.length; i++){
	   ...
	   if(["PURCHASEORDERID", "HISTORY.CREATEDBY",
	                   "HISTORY.CREATEDAT","HISTORY.CHANGEDBY",
	                   "HISTORY.CHANGEDAT","NOTEID","PARTNER",
	...
	for(var i = 0; i < rs.length; i++){
		   column = rs[i].COLUMN_NAME;
	   if(["PURCHASEORDERID", "PURCHASEORDERITEM", "PRODUCT",
